...
def visualize_trajectory_2d(pose,path_name="Dead Reckon DS10",show_ori=False):
    '''
    ...
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_title('Dead Reckon Plot For Dataset 03')
    ax.axis('equal')
    ax.grid(False)
    ax.legend()
    plt.show(block=True)
    # path = "./results"
    # name5 = "/DR_Plot/Final_DR03" + ".jpg"
...
def visualize_trajectory_2d_dead_final(pose,path_name="Dead Reckon Plots",show_ori=False):
    '''
    function to visualize the trajectory in 2D
    Input:
        pose:   4*4*N matrix representing the camera pose, 
    ...
    # plt.show(block=True)
    path = "./results"
    name5 = "/Dead_Reckon/Final_DR03" + ".jpg"
    filename5 = path + name5
    plt.savefig(filename5,dpi=300, quality=70)
    plt.close('all')
    return fig, ax
...
def visualize_trajectory_map(pose,path_name="EKF_Map",show_ori=False,m = np.eye(3),iter_no=0):
    '''
    function to visualize the trajectory in 2D
    Input:
        pose:   4*4*N matrix representing the camera pose, 
    ...
    if show_ori:
        ...
        ax.quiver(pose[0,3,select_ori_index],pose[1,3,select_ori_index],dx,dy,\
            color="b",units="xy",width=1)
    ax.scatter(x = m[0,:], y = m[1,:],s=1,color = "blue")
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.axis('equal')
    ax.grid(False)
    ...
    # plt.show(block=True)
    path = "./results"
    name5 = "/EKF_Mapping/Try1_DS10_F20_" + str(iter_no) + ".jpg"
    filename5 = path + name5
    plt.savefig(filename5,dpi=300, quality=70)
    plt.close('all')
    return fig, ax

def visualize_trajectory_SLAM(pose,path_name="EKF_SLAM",show_ori=False,m = np.eye(3),iter_no=0):
    '''
    function to visualize the trajectory in 2D
    Input:
        pose:   4*4*N matrix representing the camera pose, 
    ...
    # plt.show(block=True)
    path = "./results/"
    name5 = "/EKF_SLAM/Final2DS10" + str(iter_no) + ".jpg"
    filename5 = path + name5
    plt.savefig(filename5,dpi=300, quality=70)
    plt.close('all')
    return fig, ax

def visualize_trajectory_map_final(pose,path_name="EKF_Mapping",show_ori=False,m = np.eye(3),iter_no=0):
    '''
    function to visualize the trajectory in 2D
    Input:
        pose:   4*4*N matrix representing the camera pose, 
    ...
    # plt.show(block=True)
    path = "./results/"
    name5 = "/EKF_Mapping/Final2DS10" + str(iter_no) + ".jpg"
    filename5 = path + name5
    plt.savefig(filename5,dpi=300, quality=70)
    plt.close('all')
    return fig, ax
